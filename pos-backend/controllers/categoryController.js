 ...
 * @access  private 
 ...
 export const deleteCategory = asyncHandler(async (req , res) => {
  ...
  const itemsForCategory = await ItemsModel.find({category : req.params.categoryId});
// const publicIds = productsForCategory.map(product => product.images.public_id);
// let publicIds = [];
let publicIds = itemsForCategory.map(item => item.image.public_id)
...
if (publicIds?.length > 0) {
  ...
  await ItemsModel.deleteMany({category : req.params.categoryId});
